...
| 问题 | 当前保护 | 解决方案/发布门禁 |
|---|---|---|
| 本机 Docker Desktop 未运行，无法执行真实 Sandbox、MySQL/MinIO/Qdrant/RabbitMQ 集成和恶意样本测试 | Sandbox 客户端 fail-closed；Compose 静态配置已通过 | 启动 Docker 后执行 TEST-103/104 全部容器与故障注入；未通过不得发布 |
| 当前环境没有 `pytest` 命令 | Java 全量单测和前端构建已通过 | 在 Sandbox 镜像构建后以容器测试替代宿主 pytest，并保存镜像 digest、逃逸负例和资源限制证据 |
...
- `mvn -pl cloud-server -am test -DskipITs`：312 tests，0 failure/error/skipped。
- `npm run build`：TypeScript 与 Vite production build 通过，1832 modules transformed。
- `docker compose config --quiet`：通过。
- 设置测试用 rootless socket 后 `docker compose -f docker-compose.sandbox-runner.yml config --quiet`：通过；仅本机 Docker config 读取警告。
