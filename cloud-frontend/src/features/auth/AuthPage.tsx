...
export function AuthPage({ mode }: { mode: "login" | "sign" }) {
  ...
  const siteName = settings.data?.siteName || "YLCloud";
    ...
    <main className="auth-page">
      <section className="auth-story" aria-label="产品介绍">
        <div className="auth-story-brand">
          ...
          <div><strong>{siteName}</strong><span>Knowledge asset cloud</span></div>
        </div>
        <div className="auth-story-copy">
          <span className="auth-eyebrow">文件，从存储走向知识</span>
          <h1>让每一份资料，<br />都能被找到、协作与理解。</h1>
          <p>统一管理个人文件、团队空间与知识库，让资料自然进入解析、切分、向量化、索引与智能问答流程。</p>
        </div>
      ...
      <section className="auth-card">
        ...
        <header className="auth-card-title">
          ...
          <p>{isSign ? "注册后将自动进入知识资产工作台。" : "继续管理你的文件、空间与知识库。"}</p>
        </header>
        <form className="form-stack" onSubmit={handleSubmit(submit)} noValidate>
          ...
          <Button className="full-width" variant="confirm" size="lg" type="submit" loading={isSubmitting} disabled={isSign && !allowRegister}>
