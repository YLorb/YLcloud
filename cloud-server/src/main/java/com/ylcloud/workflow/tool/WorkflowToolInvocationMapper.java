...
public interface WorkflowToolInvocationMapper {
    @Select("select invocation_id as invocationId,run_id as runId,execution_id as executionId,node_id as nodeId," +
            "user_id as userId,session_id as sessionId,tool_name as toolName,risk_level as riskLevel," +
    ...
    @Insert("insert into workflow_tool_invocation(invocation_id,run_id,execution_id,node_id,user_id,session_id," +
            "tool_name,risk_level,arguments_hash,invocation_status,createtime,updatetime) values(" +
            "#{invocationId},#{runId},#{executionId},#{nodeId},#{userId},#{sessionId},#{toolName},#{riskLevel}," +
