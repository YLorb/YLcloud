...
import { AlertCircle, Check, CheckCircle2, ChevronDown, Clock3, Filter, RefreshCw, RotateCcw, Search, ServerCog } from "lucide-react";
...
export function TasksPage() {
  ...
  const detail = useQuery({
    ...
    enabled: Boolean(selectedTask && (selectedTask.source === "rag" || selectedTask.source === "knowledge") && Number.isFinite(selectedTaskId)),
...
function TaskDetail({ task, onRetry, retrying }: { task: AsyncTaskDetail; onRetry: () => void; retrying: boolean }) { const status = taskStatus(task); return <div className="task-detail"><section className={`task-detail__summary task-detail__summary--${status.tone}`}>{status.key === "success" ? <CheckCircle2 size={22} /> : status.key === "failed" ? <AlertCircle size={22} /> : <Clock3 size={22} />}<div><StatusBadge tone={status.tone}>{status.label}</StatusBadge><strong>{task.completionSummary || task.message || status.label}</strong>{task.terminalReason && <p>{task.terminalReason}</p>}</div></section><dl className="task-detail__facts"><div><dt>执行阶段</dt><dd>{task.terminalStage || task.phase || "—"}</dd></div><div><dt>进度</dt><dd>{taskProgress(task)}%</dd></div><div><dt>成功 / 失败</dt><dd>{task.successCount ?? 0} / {task.failedCount ?? 0}</dd></div><div><dt>耗时</dt><dd>{formatDuration(task.durationMs)}</dd></div><div><dt>开始时间</dt><dd>{formatTime(task.startedTime || task.createTime)}</dd></div><div><dt>完成时间</dt><dd>{formatTime(task.finishedTime || task.updateTime)}</dd></div></dl>{task.errorMessage && <section className="task-detail__error" role="alert"><AlertCircle size={18} /><div><strong>失败原因</strong><p>{task.errorMessage}</p></div></section>}{status.key === "failed" && task.retryable !== false && <div className="task-detail__actions"><Button variant="danger" loading={retrying} onClick={onRetry}><RotateCcw size={15} />重试任务</Button></div>}</div>; }
