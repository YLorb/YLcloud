...
@Component
public class OcrStructuredParser {
    private final RagProperties ragProperties;
    private final ParserServiceClient parserServiceClient;
    private final MinioclientUtil minioclientUtil;

    public OcrStructuredParser(RagProperties ragProperties, ParserServiceClient parserServiceClient, MinioclientUtil minioclientUtil) {
    ...
    public ParsedDocument parse(SpaceFile spaceFile, File file) {
        ...
        try {
            ...
            return ParsedDocument.failed(fileUuid(spaceFile,file),fileHash(file),"ocr-layout",parserVersion,
                    "Failed to create parser object URL: " + ex.getMessage());
        }
        return parserServiceClient.parseOcr(new ParserServiceClient.OcrParseRequest(
