...
- 发现时间：2026-07-17。
- 类型：新增功能需求；P0～P2 核心能力已实现，后续问题按本节确认的产品决策继续收口。
...
- 当前能力：服务端已按 `sessionId/userId` 从持久化消息构建权威短期上下文，并保存 token 预算、滚动摘要和可复用的 context snapshot；长期记忆已有独立实体、用户级向量索引、提取/召回、冲突替换与删除补偿；情景层保留完整消息事件，并生成固定窗口 Episode。前端不再负责上传可信 history。
...
| 长期记忆 Long-term Semantic Memory | 从已完成对话中提取、去重并可由用户管理的稳定事实/偏好/约束 | 跨会话永久保留，直到被覆盖或由用户主动删除 | 使用现有 embedding 和 RAG 检索链召回，再作为“用户记忆”独立注入 | 跨会话记住用户偏好、长期项目背景和已确认决策 |

“情景记忆”按一个会话线程定义一个 episode，不等同于前端 Dialog 弹窗组件。长会话可按主题变化或固定消息窗口拆成多个 episode，但原始消息顺序仍以数据库事件流为准。

#### 2026-07-20 产品决策与 Episode 说明

- `Episode` 是长会话的“对话片段/章节”：服务端按消息顺序保存片段的起止位置、标题与摘要，前端将其显示为横向目录，用户可直接跳转到对应消息。它不是新的记忆类型，也不是弹窗组件。
- 当前 Episode 采用每 20 条有效消息一个片段的确定性基线。该模型用于导航和来源追溯；主题识别、决策、待办与未解决问题的结构化拆分不属于本次功能闭环。
- 用户反馈只保留后端接口和数据表，供质量分析与离线评测使用；Assistant 前端不提供点赞、点踩或反馈表单。
- 关闭长期记忆只停止后续写入与召回，不删除已有记忆，也不要求用户在关闭时作额外选择。
- 长期记忆默认永久保留，不执行自动过期。只有用户在“记忆管理”页面主动忘记单条或清空全部时，才进入清理流程；关闭记忆或删除来源会话均不清理长期记忆。
- 长期记忆 Rerank 采用失败关闭语义：结果为 `null`、空列表、分数缺失或全部低于阈值时，返回非 `null` 的空列表，表示召回已完成但没有合格记忆；禁止回退到未经 Rerank 阈值确认的向量候选。
...
- 删除或编辑会话时同步失效短期快照和 Episode，但不级联删除由该会话派生的长期记忆。来源会话删除后，长期记忆仍作为独立用户数据保留，直到用户在记忆管理页面主动清理。
- 长期记忆、短期快照和情景记录均按 `userId` 隔离。共享 Space 成员不能看到彼此的个人记忆；管理员管理配额和开关，但普通管理接口不返回记忆正文。
- 用户需要“记忆管理”页面：总开关、分类筛选、查看来源、编辑、固定、忘记单条、清空全部和导出。短期/情景记忆作为对话连续性的基础默认开启；长期记忆默认开启，可随时关闭。关闭后停止写入和召回，既有记忆继续长期保存，清理只能由用户在记忆管理页面主动发起。
...
- P1：长期记忆候选提取、独立数据模型、现有 embedding 复用、用户级 Qdrant 检索、冲突替换/删除状态机和跨存储补偿；不执行自动过期。
- P2：记忆管理 UI、Episode 导航、后端反馈采集、评测集、监控与成本优化；反馈不提供前端入口。
...
- 删除会话必须保留长期记忆；删除单条记忆、清空记忆、并发更新、重复任务、embedding 失败、Qdrant 超时和进程退出均有故障注入，最终 MySQL/Qdrant 一致。
