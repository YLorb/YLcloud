...
import { Bot, Check, ChevronLeft, FileText, MessageSquarePlus, Plus, Search, Send, Sparkles, Trash2, User } from "lucide-react";
...
export function AssistantPage() {
  ...
  const activeSession = useQuery({ queryKey: ["chat-session", sessionId], queryFn: () => api.getKnowledgeChatSession(sessionId!), enabled: Boolean(sessionId) });
  useEffect(() => { if (!sessionId && sessions.data?.[0]) setSessionId(sessions.data[0].id); }, [sessionId, sessions.data]);
  useEffect(() => { if (activeSession.data?.spaceIds?.length) setSelectedSpaces(activeSession.data.spaceIds); }, [activeSession.data]);
  useEffect(() => messageEnd.current?.scrollIntoView({ behavior: "smooth" }), [activeSession.data?.messages]);
  ...
  const send = useMutation({
    mutationFn: async () => {
      ...
      await api.appendKnowledgeChatMessage(currentSessionId, { role: "user", content });
      const history = (activeSession.data?.messages || []).slice(-10).map<RagChatMessage>((message) => ({ role: message.role, content: message.content }));
      const result = await api.queryKnowledgeRag({ spaceIds: selectedSpaces, question: content, retrievalMode: "balanced", history });
      const answer = result.answer?.trim() || "当前知识库中没有足够相关的信息来回答这个问题。";
      await api.appendKnowledgeChatMessage(currentSessionId, { role: "assistant", content: answer, citationsJson: JSON.stringify(result.citations || []) });
