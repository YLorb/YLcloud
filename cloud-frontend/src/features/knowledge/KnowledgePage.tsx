...
function statusPresentation(status?: string | number): { tone: StatusTone; label: string } { const value = String(status ?? "").toUpperCase(); if (["ACTIVE", "VALID", "SUCCESS", "COMPLETED", "DONE", "INDEXED", "READY", "2"].includes(value)) return { tone: "success", label: value === "ACTIVE" || value === "VALID" ? "已生效" : "成功" }; if (["FAILED", "ERROR", "ROLLED_BACK", "-1"].includes(value)) return { tone: "danger", label: value === "ROLLED_BACK" ? "已回滚" : "失败" }; if (value === "NEEDS_REVIEW") return { tone: "warning", label: "待复核" }; if (["RUNNING", "PROCESSING", "1"].includes(value)) return { tone: "running", label: "处理中" }; if (["PENDING", "WAITING", "0"].includes(value)) return { tone: "warning", label: "等待中" }; if (value === "SUPERSEDED") return { tone: "neutral", label: "已被替代" }; return { tone: "neutral", label: status ? String(status) : "未知" }; }
