...
| 将 Docker Socket 挂入 Sandbox 服务容器会赋予其宿主 Docker 的 root 等价控制能力 | 本轮未重建该高权限运行方式；改由 Win10 宿主进程直接编排一次性受限工具容器完成验证 | 生产必须使用独立 rootless Docker/隔离节点，不得把 Docker Desktop 主 Socket 暴露给对外服务；Compose runner 仅用于受控专用执行节点 |
